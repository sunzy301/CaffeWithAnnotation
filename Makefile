...
# 配置文件名 
# makefile.config文件，单独文件记录所有编译需要配置的内容
CONFIG_FILE := Makefile.config
# Explicitly check for the config file, otherwise make -k will proceed anyway.
ifeq ($(wildcard $(CONFIG_FILE)),)
# 检查配置问价是否存在，如果不存在，使用config.example  
$(error $(CONFIG_FILE) not found. See $(CONFIG_FILE).example.)
endif
# 包含配置文件
include $(CONFIG_FILE)

# BUILD_DIR在Makefile.config文件中定义　caffe中的build目录  
BUILD_DIR_LINK := $(BUILD_DIR)
# 是编译方式，使用debug还是release编译，如果用debug生成./build_debug目录，release生成./build_release目录
...
# 检查debug是否为０,debug的定义在.config文件中 
DEBUG ?= 0
ifeq ($(DEBUG), 1)
...
# $(Q)是关于是否有@，表示是否会输出整个cc命令，在config文件中定义
# $@是目标集合
# $^依赖的集合
