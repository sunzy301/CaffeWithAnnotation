...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  // 创建每个layer，包括初始化和参数blob创建
  // 使用top和bottom blob连接不同的layer
  for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
    // For non-root solvers, whether this layer is shared from root_net_.
    // 对于非根solver，需要从父类中继承网络结构
    ...
    // 先生成bottom，再是top
    // 最底层的bottom都一定是input
      // 具体生成blob的操作一定都是在top中，bottom都是共享低层的top blob
    ...
    if (share_from_root) {
      ...
      // 指定bottom和top blob，自定义初始化layer操作
      // 调用layer的setup函数
  ...
  // 最后剩余的就是output blobs
    // 是top blob但不是bottom blob的blob会留在available_blobs这个set中
...
void Net<Dtype>::AppendTop(const NetParameter& param, const int layer_id,
  ...
  if (blob_name_to_idx && layer_param && layer_param->bottom_size() > top_id &&
    ...
    // raise an error.
    // 如果不是址内计算，top blob应该是需要创建出来的，而不是已经生成的
...
// 添加一个bottom blob
// bottom blob一定是共用别的top blob，所以一定已经创建过了
template <typename Dtype>
...
void Net<Dtype>::AppendParam(const NetParameter& param, const int layer_id,
  ...
  // net_param_id是参数blob在全局net中参数的编号
  const int net_param_id = params_.size();
  // layer类中的blobs返回一个vector<Blob>，是参数blob的vector
  // params_不管参数blob的任何性质都会收集这个参数blob指针
...
// 清除learnable_params_中的内容，就是清除上一轮计算的梯度值
template <typename Dtype>
